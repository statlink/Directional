mixpkbd.mle2 <- function(x, g, n.start = 10, tol = 1e-6, maxiters = 100) {

  d <- dim(x)[2]
  runtime <- proc.time()
  mod <- flexmix::initFlexmix( x ~ 1, k = g, model = circlus::FLXMCpkbd(),
                               control = list(minprior), nrep = n.start )
  mod <- flexmix::flexmix( x ~ 1, cluster = mod@cluster, model = circlus::FLXMCpkbd(),
