bic.mixspcauchy <- function(x, G = 5, n.start = 5, tol = 1e-6, maxiters = 500) {
  ...
  for ( vim in 2:G ) {
    a <- Directional::mixspcauchy.mle(x, vim, n.start = n.start, tol = tol, maxiters = maxiters)  
  ...
  plot(1:G, icl, col = ina, xlab = "Number of components", ylab = "BIC values", cex.lab = 1.3, cex.axis = 1.3)
  abline(v = 1:G, lty = 2, col = "lightgrey")
  abline(h = seq(min(icl, na.rm = FALSE), max(bic, na.rm = FALSE), length = 10), lty = 2, col = "lightgrey" )
  lines(1:G, icl, lwd = 2)
