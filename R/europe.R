...
europe <- function(title = "Europe", coords = NULL) {
...
# Select what to plot
world <- ne_countries(continent = "Europe",
                      scale = "medium",
                      returnclass = "sf") %>%
...
# For adding more countries do then i.e.
world <- ne_countries( country = c( world$name, "Turkey", "Cyprus"),
                       scale = "medium",
                       returnclass = "sf") %>%
#
#         # Set map projection
...
ggplot() +
  ...
  # map margins
  xlim(c(-1299950, 3000000)) +
  ylim(c(4199900, 7750000)) +
