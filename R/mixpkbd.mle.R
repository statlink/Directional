mixpkbd.mle <- function(x, g = 2, n.start = 5, tol = 1e-6, maxiters = 100) {
  ...
  fun2 <- function(wlika, rswlika, mu, x, d, g, lika, tol, maxiters) {
    ...
    for (j in 1:g) {
      ...
      com <- sqrt(g2 + 1)
      com2 <- 1 / (com - a)
      lika[, j] <- 0.5 * (d + 1) * log( com2 ) - 0.5 * (d - 1) * ( log(com - 1) - log(g2) ) + log(pj[j])
    }
  ...
  if ( min(wij) <= 3 ) {
    ...
    for (j in 1:g) {
      #mod <- Directional::pkbd.mle(x[cl == j, ])
      #g2 <- mod$gamma^2
      #mu[j, ] <- mod$mesos
      #a <- as.vector(x %*% mu[j, ])
      #com <- sqrt(g2 + 1)
      #com2 <- 1 / (com - a)
      #lika[, j] <- 0.5 * (d + 1) * log( com2 ) - 0.5 * (d - 1) * ( log(com - 1) - log(g2) )
      mod <- Directional::pkbd.mle2(x[cl == j, ])
...
.wpkbd.wmle <- function(x, w, tol = 1e-6, maxiters = 100) {
  ...
  pkbd <- function(rho, mu, x, n, d, w, sw) {
    a <- as.vector(x %*% mu)
    sw * log( 1 - rho^2 ) - 0.5 * d * sum( w * log1p( rho^2 - 2 * rho * a ) )
  }
