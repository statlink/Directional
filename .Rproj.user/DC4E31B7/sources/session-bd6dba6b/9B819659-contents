spher.purka.contour <- function(theta, a, bgcol = "snow", dat = NULL, col = NULL, lat = 50, long = 50) {
  ...
  mat <- Directional::dpurka(wa, a, theta)   # uses parameter 'a' correctly

  mat <- matrix(mat, nrow = n1, ncol = n2, byrow = TRUE)

  finite_vals <- mat[ is.finite(mat) ]
  levs <- quantile(finite_vals, probs = seq(0.5, 0.97, length.out = 7))
  levs <- unique(levs)
  ## Bug 4 fix: renamed 'a' -> 'clines' to avoid overwriting the parameter 'a'
  clines <- contourLines(x1, x2, mat, levels = levs)
  ...
  for ( i in seq_along(clines) ) {
    y1 <- Directional::euclid( cbind(clines[[i]]$x, clines[[i]]$y) ) * 1.002
